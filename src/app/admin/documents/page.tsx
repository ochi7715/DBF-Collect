  ...
  return (
    <section className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-slate-600">Review uploaded race registration forms, waivers, and seating charts.</p>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {docs.map((doc) => (
              <tr key={doc.id} className="align-top">
                <td className="px-4 py-3 font-semibold text-slate-900">{doc.teams.name}</td>
                <td className="px-4 py-3 text-slate-600">{doc.team_members?.full_name ?? "-"}</td>
                <td className="px-4 py-3 text-slate-700">{doc.document_forms.name}</td>
            ...
            {docs.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-500">No submitted documents yet.</td>
              </tr>
            ) : null}
          </tbody>
          </table>
