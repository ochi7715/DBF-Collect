      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-col justify-between gap-4 md:flex-row md:items-start">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-slate-950">{template.name}</h1>
            <p className="mt-2 text-slate-600">{child.first_name} {child.last_name} · Updated {formatDate(doc.updated_at)}</p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {doc.file_path ? (
            ...
            <p className="mt-4 text-sm text-slate-600">No file uploaded for this checklist item.</p>
          )}

          {doc.signed_file_path ? (
            <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              <p className="font-semibold text-emerald-900">Signed document stored</p>
              <p className="mt-1 text-sm text-emerald-700">The completed PDF has been downloaded from Dropbox Sign and stored privately.</p>
        ...
        <div className="space-y-4">
          ...
          {template.requires_signature ? (
            <form action="/api/dropbox-sign/send" method="post" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-600">Send the configured Dropbox Sign template to the caregiver signer.</p>
